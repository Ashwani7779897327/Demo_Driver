...
public class RentalTrackRideActivity extends BaseActivity implements OnMapReadyCallback, ApiManager.APIFETCHER {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        trip_status_txt.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (rideSession.getCurrentRideDetails().get(RideSession.RIDE_STATUS).equals("11")) {  // run arrived API_S
                    HashMap<String, String> data = new HashMap<String, String>();
                    data.put("rental_booking_id", "" + rideSession.getCurrentRideDetails().get(RideSession.RIDE_ID));
                    data.put("driver_id", "" + sessionManager.getUserDetails().get(SessionManager.KEY_DRIVER_ID));
                ...
                if (rideSession.getCurrentRideDetails().get(RideSession.RIDE_STATUS).equals("12")) {  // run begin trip API_S
...
//                Log.d("" + TAG, "Data Fetched from firebase cancelled " + databaseError.getMessage());
//            }
//        });
//    }
//
