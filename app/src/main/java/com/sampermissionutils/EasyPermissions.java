...
/**
 * Utility to request and check System permissions for apps targeting Android M (API_S >= 23).
 */
public class EasyPermissions {
    ...
    public static boolean hasPermissions(Context context, @NonNull String... perms) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            Log.w(TAG, "hasPermissions: API_S version < M, returning true by default");

            // DANGER ZONE!!! Changing this will break the library.
            return true;
        }

        // Null context may be passed if we have detected Low API_S (less than M) so getting
